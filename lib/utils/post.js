...
class Post {
  ...
  _readContent() {
    ...
    const noMore = noFront
      .replace('<!--more-->', '')
      .replace('<!-- more -->', '');
    this.contentHtml = md.render(noMore);
  }
  ...
  // public
  loadConfig({ codeTheme }) {
    this.codeTheme = codeTheme;
    return this;
  }
  loadTemplate({ layoutTemplate, postTemplate, itemTemplate }) {
    this.layoutTemplate = layoutTemplate;
    this.postTemplate = fs.readFileSync(postTemplate, 'utf-8');
    this.itemTemplate = fs.readFileSync(itemTemplate, 'utf-8');
    return this;
  }
